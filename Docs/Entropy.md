...
- 수정) Entropy는 0 ~ 1 사이의 값을 가진다고 표현하면 안된다. 예측이 몇개인지에 따라 다르다.
...
- 예측과 정답이 얼마나 근사한가에 대한 값이며 예측이 정답이 가까울 수록 실제 엔트로피와 같아진다.
...
( 예측이 실제 값과 같아지는 방향으로, 엔트로피 값과 같아지는 방향으로)
...
- 정답과 예측 분포가 상대적으로 얼마나 차이가 있는지에 대한 것
( CrossEntropy - Entropy의 수치이기 때문에 0에 수렴할 수록 예측과 정답이 같다.)

- Entropy는 변하지 않고 CrossEntropy는 Entropy로 수렴하는 것이 목적이므로 Entropy가 상수라서 KL Divergence를 평가지표로 쓰나 CrossEntropy를 쓰나 결국 같은 현상이다.
...
[허민석유튜브](https://www.youtube.com/@TheEasyoung)